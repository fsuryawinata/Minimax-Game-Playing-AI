...
def minimaxDecision(depth, game):
    ...
    for op in getOperators(game):
        ...
        if value > best_value:
            best_value = value
            best_operator = op
            print(f"BEST OP {best_operator}, BEST VAL {best_value}")
...
def minimaxValue(state, game, depth, alpha, beta):
    ...
    if state.game_over or depth == 0:
        # print(f"GAME OVER? {game_over(state)} VALUE {utility(state)} GAME TURN IS {game.turn_color}")
        ...
        if game.turn_color == state.turn_color:
            ...
            for op in getOperators(state):
                ...
                new_state.apply_action(op)
                # print(f"min op {op}")
                # print(f"{new_state._state.items()}")
                # Player turn, depth = 1
...
def utility(state):
    ...
    # Switch color since game turn has changed
    # Opponent was player
    switchColour(state)
    ...
    print(f"PLAYER POW {player_power} + OPP POW {opponent_power} +"
